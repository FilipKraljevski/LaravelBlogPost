        ...
        <div class="panel-body">
            @if(!empty($latest_posts[0]))
                @foreach($latest_posts as $latest_post)
                    <p>
                        <strong><a href="{{ route('posts.show', [$latest_post->slug]) }}">{{ $latest_post->title }}</a></strong>
            ...
            @if(!empty($latest_comments[0]))
                @foreach($latest_comments as $latest_comment)
                    <div class="list-group-item">
                        ...
                        <p>On post <a href="{{ route('posts.show', [$latest_comment->post->slug]) }}">{{ $latest_comment->post->title }}</a></p>
                    </div>
                @endforeach
            @else
                <div class="list-group-item">
